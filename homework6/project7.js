...
function SimTimeStep(dt, positions, velocities, springs, stiffness, damping, particleMass, gravity, restitution) {
    ...
    const forces = Array(numParticles).fill().map(() => gravity.copy().mul(particleMass));
    ...
    for (const spring of springs) {
        ...
        const dir = dx.div(length); 

        const springForce = dir.mul(stiffness * (length - spring.rest));

        const vRel = vj.sub(vi);
        const dampingForce = dir.mul(damping * vRel.dot(dir));
